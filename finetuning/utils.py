...
class Sight(LanguageModel):
    ...
    def __init__(self, model, tokenizer=None, *args, **kwargs):
        # Fix: Pass tokenizer separately, don't try to access model.tokenizer
        super().__init__(model, tokenizer=tokenizer, *args, **kwargs)
    ...
    def __getitem__(self, *args):
        ...
        # Fix: Support different model architectures including Llama
        # Check if model has 'transformer' or 'model'
        if hasattr(self._envoy, 'transformer'):
            # GPT-style models
            return dict(
        ...
        elif hasattr(self._envoy, 'model'):
            # Llama-style models
            return dict(
                resid_pre=self._envoy.model.layers[layer].input,
                resid_post=self._envoy.model.layers[layer].output,
                mlp_in=self._envoy.model.layers[layer].mlp.input,
                mlp_out=self._envoy.model.layers[layer].mlp.output,
                attn_out=self._envoy.model.layers[layer].self_attn.output,
                pattern=self._envoy.model.layers[layer].self_attn.softmax.output,
                scores=self._envoy.model.layers[layer].self_attn.softmax.input[0],
            )[point]
        else:
            raise AttributeError("Model architecture not supported. Expected 'transformer' or 'model' attribute.")